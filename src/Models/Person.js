...
class Person {
  ...
  isEmpty() {
    return deepEqual(this, new Person());
  }
  ...
  isIdentityCard() {
    return this.credential.type === IdentityCard.type;
  }
  ...
  setCredential(credential, changes) {
    if (!credential) {
      ...
      if (IdentityCard.isValid(this.credential.number, true)) {
        this.refreshGenderBirthday();
        if (changes) {
  ...
  validate(field) {
    if (!field || field === '*') {
      ...
      switch (field) {
        case 'name':
          return Name.validResult(this.name, this.name);
        case 'credential':
        ...
        case 'gender':
          return new ValidationResult(!!this.gender, `请选择${this.name || ''}性别`);
        case 'birthday':
          return new ValidationResult(!!this.birthday, `请选择${this.name || ''}出生日期`);
        case 'mobile':
          return Mobile.validResult(this.mobile, this.name);
        default:
