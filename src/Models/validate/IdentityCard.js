...
import { CREDENTIAL_TYPE_MAP } from '@/models/common/CredentialType';
...
export default {
  ...
  isValid(number) {
    ...
    return isIdCardNumberValid(number) || identity15Reg.test(number);
  },

  validResult(number, name = '') {
    if (!number) {
      return new ValidationResult(false, `${name}证件号码不能为空`);
    }
    return new ValidationResult(this.isValid(number, true), `${name}证件号码错误，请进行核对并修改`);
  },

  getGender(number) {
    const GENDER_INDEX = number?.length === 15 ? 14 : 16;
    const digit = number.charCodeAt(GENDER_INDEX) - 48;
    return (digit % 2 === 1 ? GENDER_MAP.MALE.value : GENDER_MAP.FEMALE.value);
  },

  getBirthday(number) {
    const isOldIdentityCard = number?.length === 15;
    let year;
    let month;
    let day;
    if (isOldIdentityCard) {
      year = `19${number.substring(6, 6 + 2)}`;
      month = number.substring(8, 8 + 2);
      day = number.substring(10, 10 + 2);
    } else {
      year = number.substring(6, 6 + 4);
      month = number.substring(10, 10 + 2);
      day = number.substring(12, 12 + 2);
    }
